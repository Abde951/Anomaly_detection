...
def save_tsne(test_features, val_labels_binary, classe_path):
    ...
    plt.savefig(classe_path + '/tsne.png')
...
########## KNN ########
def save_validation_losses_density_KNN(val_losses, val_labels,classe_path, latent_space, k):
    if not os.path.exists(classe_path):
            os.makedirs(classe_path)
    df = pd.DataFrame(val_losses, columns=['loss'])
    df['labels'] = val_labels
    #df_def = pd.DataFrame(losses_def, columns=['loss_def'])
    sns.displot(df,x="loss" ,hue="labels")
    save_path = classe_path + f"/Density_latent_dim_{latent_space}_k_{k}.png"
    plt.savefig(save_path)


def save_confusion_matrix_KNN(labels, predictions,classe_path, latent_space, k):
    if not os.path.exists(classe_path):
            os.makedirs(classe_path)
    classes = ['normal','deformed']
    matrix = confusion_matrix(labels, predictions, labels=classes)
    ...
    ax.xaxis.set_ticklabels([classes[0], classes[1]])
    ax.yaxis.set_ticklabels([classes[0], classes[1]])
    plt.savefig(classe_path + f"/confusion_matrix_latent_dim_{latent_space}_k_{k}.png")


def save_roc_curve_KNN(val_labels_binary, anomaly_scores, latent_space, classe_path, k):
    plt.figure()
        # PLOTING ROC CURVE
    fpr, tpr, _ = roc_curve(val_labels_binary,  anomaly_scores)
    ...
    plt.ylabel('True Positive Rate')
    plt.xlabel('False Positive Rate')
    plt.savefig(classe_path + f'/roc_curve_latent_dim_{latent_space}_k_{k}.png' )